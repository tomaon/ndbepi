...
call() ->
    ...
    {ok, L1} = ndbepi_gen_block:call(P, {get_table_by_name, [<<"test/def/city">>], 0}),
    [io:format("~p~n", [E]) || E <- L1 ], io:format("~n~n"),
    %%{ok, L2} = ndbepi_gen_block:call(P, {get_table_by_id, [1], 0}), % DEFAULT-HASHMAP-3840-2
    %%[io:format("~p~n", [E]) || E <- L2 ], io:format("~n~n"),
    ok.

%% -- behaviour: ndbepi_gen_block --

init([NodeId, BlockNo]) ->
    {ok, #data{node_id = NodeId, block_no = BlockNo, request_id = 0}}.
...
attibute() ->
    [
     ...
     %%ATTRIBUTE_TYPE, <<"Type">>},
     {?ATTRIBUTE_SIZE, <<"Size">>},
     {?ATTRIBUTE_ARRAY_SIZE, <<"ArraySize">>},
     {?ATTRIBUTE_KEY_FLAG, <<"KeyFlag">>},
     {?ATTRIBUTE_STORAGE_TYPE, <<"StorageType">>},
     {?ATTRIBUTE_NULLABLE_FLAG, <<"NullableFlag">>},
     %%ATTRIBUTE_DYNAMIC, <<"Dynamic">>},
     {?ATTRIBUTE_D_KEY, <<"DKey">>},
     {?ATTRIBUTE_EXT_TYPE, <<"ExtType">>},
     {?ATTRIBUTE_EXT_PRECISION, <<"ExtPrecision">>},
     {?ATTRIBUTE_EXT_SCALE, <<"ExtScale">>},
     {?ATTRIBUTE_EXT_LENGTH, <<"ExtLength">>},
     {?ATTRIBUTE_AUTO_INCREMENT, <<"AutoIncrement">>},
     {?ATTRIBUTE_ARRAY_TYPE, <<"ArrayType">>},
     {?ATTRIBUTE_DEFAULT_VALUE_LEN, <<"DefaultValueLen">>},
